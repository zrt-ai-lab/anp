...
PEER_AD_URL = os.getenv("CHAT_PEER_AD_URL", "http://localhost:8000/a/ad.json")
...
class ChatAgentB:
    ...
    async def notify_connected(self, agent: str) -> dict:
        ...
        return {"ok": True, "agent": "ChatB", "connected": agent_name}
    ...
    if AUTO_DISCOVER:
        async def _auto_discover_loop():
            for _ in range(max(1, AUTO_DISCOVER_MAX_TRIES)):
                if await chat_agent_b.ensure_peer_connection():
                    if not chat_agent_b._auto_start_attempted:
                        chat_agent_b._auto_start_attempted = True
                        await chat_agent_b.maybe_start_chat_if_discovered_first(turns=AUTO_CHAT_TURNS)
                    return
...
async def root():
    return {
        ...
        "messages_received": chat_agent_b.message_count,
        "peer_connected": chat_agent_b.peer is not None,
        "peer_name": chat_agent_b.peer_name,
...
async def p2p_discover(payload: dict = Body(default={})):  
    ...
    ok = await chat_agent_b.ensure_peer_connection(peer_ad_url=peer_ad_url)
    if ok:
        await chat_agent_b.maybe_start_chat_if_discovered_first(turns=AUTO_CHAT_TURNS)
    return {
        "ok": ok,
        "peer_ad_url": (peer_ad_url or "").strip() or PEER_AD_URL,
        "peer_name": getattr(chat_agent_b.peer, "name", None),
    }
...
async def p2p_send(payload: dict = Body(default={})): 
    ...
    resp = await chat_agent_b.send_message(
        message=str(message),
